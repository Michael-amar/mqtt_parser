...
class PublishVariableHeader(MQTTVariableHeader):
    ...
    def to_bytes(self):
        attributes = ['topic_name', 'packet_identifier', 'properties']
        return b''.join([instance.to_bytes() for instance in [getattr(self, attr) for attr in attributes] if instance is not None])
    
    @classmethod
    def from_args(cls, packet_identifier : int, topic_name : str, properties : list[Property]):
        ...
        length = len(identifier.to_bytes()) + len(topic.to_bytes()) + len(props.to_bytes())
        kwargs = {
